...
Milestone 6 data-plane tests call the tonic service directly with `tonic::Request`, not fixed ports. They cover publish success, unknown and invalid topics, empty payloads, consume metadata mapping, invalid consumer groups, invalid `max_messages`, invalid `lease_ms`, empty consumes, in-flight lease no-redelivery, ACK success and stale/unknown/wrong-consumer errors, ACK durability after reopen, NACK unknown delivery, retry through broker retry maintenance, DLQ routing, and a full publish/consume/ACK durable reopen flow.

## End-to-End Tests

Future E2E tests will launch the broker runtime and use CLI/SDK flows for create topic, publish, consume, ACK/NACK, retries, and DLQ inspection. Milestone 5 adds runtime smoke coverage for `brokerd --version`, `brokerd serve --help`, and invalid listen-address parsing. Milestone 6 adds `brokerd serve-grpc --help` and invalid gRPC listen-address smoke coverage, but it does not add process-level gRPC E2E tests yet.
